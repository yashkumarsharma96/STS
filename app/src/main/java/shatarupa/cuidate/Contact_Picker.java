  ...
  extends AppCompatActivity
{
  ...
  public void onCreate(Bundle savedInstanceState)
  {
    ...
    //Reset Contacts
      ...
      int numberOfcontacts=settings.getInt("numberOfEmergencyContacts",0);
      if(numberOfcontacts!=0) {
          for (int x = 0; x < numberOfcontacts; x++) {
              settings.edit().remove("emConName" + x);
              settings.edit().remove("emConNum" + x);
              settings.edit().commit();
          }
      }
      editor.putInt("numberOfEmergencyContacts", 0);
      numberOfcontacts=0;
    ...
    {
      Toast.makeText(this, "Permission to read contacts not granted!! Please provide the required permissions and try again!!", Toast.LENGTH_LONG).show();
